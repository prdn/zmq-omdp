...
Worker.prototype.onRequest = function(clientId, rid, data) {
	...
	reply.error = function(err) {
		self.replyError(clientId, rid, self.encode(err));
		self.wait();
	};
...
Worker.prototype.replyPartial = function(clientId, rid, data) {
    ...
    this.send([MDP.WORKER, MDP.W_REPLY_PARTIAL, clientId, rid, null, data]);
	return true;
};
...
Worker.prototype.replyFinal = function(clientId, rid, data) {
    ...
    this.send([MDP.WORKER, MDP.W_REPLY, clientId, rid, null, data]);
	return true;
};
...
Worker.prototype.replyError = function(clientId, rid, err) {
	...
	this.send([MDP.WORKER, MDP.W_REPLY, clientId, rid, err]);
	return true;
};
