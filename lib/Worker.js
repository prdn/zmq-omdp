...
Worker.prototype.connectToBroker = function () {
	...
	self.name = 'Worker-' + process.pid + Math.ceil(new Date().getTime() * Math.random() * 1000);
...
Worker.prototype.replyPartial = function (client, rid, data) {
    ...
    this.socket.send([MDP.WORKER, MDP.W_REPLY_PARTIAL, client, '', rid, JSON.stringify(rep)]);
	return true;
};
...
Worker.prototype.replyFinal = function (client, rid, data) {
    ...
    this.socket.send([MDP.WORKER, MDP.W_REPLY, client, '', rid, JSON.stringify(rep)]);
	return true;
};
