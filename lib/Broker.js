...
Broker.prototype.requestUnqueue = function(serviceName) {
	...
	while(!req && service.requests.length) {
		req = service.requests.shift();
		if (!this.reqs[req.rid]) {
			req = null;
		}
	}
...
Broker.prototype.requestPurge = function(serviceName, msg) {
	...
	Object.keys(this.reqs).every(function(rid) {
		...
		if (req.timeout > -1 && ((new Date()).getTime() > req.ts + req.timeout)) {
			self.requestDelete(rid, true, 'B_TIMEOUT');
		}
...
Broker.prototype.serviceDispatch = function(serviceName) {
    ...
    while (service.waiting.length && service.requests.length) {
		...
		this.socket.send([workerId, MDP.WORKER, MDP.W_REQUEST, req.clientId, req.msg[4], req.msg[5]]);	
		this.socket.send([req.clientId, MDP.CLIENT, MDP.W_HEARTBEAT, req.rid]);
		debug("Dispatched: workerId = %s, rid = %s", workerId, req.rid);
    }
