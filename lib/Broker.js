...
Broker.prototype.start = function(cb) {
	...
	this.hbTimer = setInterval(function() {
		...
		Object.keys(self.workers).every(function(workerId) {
			...
			if (worker.rid) {
				var req = self.reqs[worker.rid];
				if (self.clients[req.clientId]) {
					obj.push(req.clientId, worker.rid);
				}
			}
		...
		self.clientPurge();
		Object.keys(self.clients).every(function(clientId) {
			self.socket.send([clientId, MDP.CLIENT, MDP.W_HEARTBEAT]);
			return true;
		});
    }, HEARTBEAT_INTERVAL);
...
Broker.prototype.onClient = function(msg) {
	...
	if (type == MDP.W_REQUEST) {
		...
		client.liveness = HEARTBEAT_LIVENESS;
	}
};
...
Broker.prototype.requestDelete = function(rid, notify, err) {
	...
	if (notify) {
		this.socket.send([rme.clientId, MDP.CLIENT, MDP.W_REPLY, rid, JSON.stringify({ status: 500 || err })]);
	}
