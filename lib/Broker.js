...
Broker.prototype.onClient = function (msg) {
	...
	if (type == MDP.W_REQUEST) {
		...
		debug("B: REQUEST from client: %s, service: %s", msg[0].toString(), serviceId);
...
Broker.prototype.onWorker = function (msg) {
	...
	if (type == MDP.W_READY) {
		...
		debug('B: register worker: %s, service: %s', identify, service, workerReady ? 'R' : '');
		if (workerReady) {
		...
		// debug('B: HEARTBEAT from %s', identify);
		if (workerReady) {
